#include <stdio.h>
#include <stdlib.h>
#include <string.h>

/*
    TO DO LIST
-̶-̶>̶ S̶e̶t̶ C̶u̶r̶r̶e̶n̶t̶ U̶s̶e̶r̶ i̶n̶ b̶o̶t̶h̶ s̶i̶g̶n̶ u̶p̶ a̶n̶d̶ L̶o̶g̶ i̶n̶
--> Deal with Change username and Change Password in Customer.c
-̶-̶>̶ C̶t̶r̶l̶+̶C̶ C̶t̶r̶l̶+̶V̶ s̶i̶m̶i̶l̶a̶r̶ c̶o̶d̶e̶ o̶f̶ i̶t̶e̶m̶s̶ and cart
--> Find the total off the bill according to price
*/

struct user_details
{
    int acc_no;
    char username[50];
    char password[50];
    struct user_details *next;
} *user_database;

struct item_details
{
    int item_no;
    char item_name[50];
    int price;
    int quantity;
    struct item_details *next;
} *item_database, *cart_items;

typedef struct user_details user_t;
typedef struct item_details item_t;

user_t error_details = {-1,"-1","-1"};
item_t error_items = {-1,"-1",-1,-1};

#include "invalid_func.c"
#include "admin.c"
#include "customer.c"
#include "exit.c"

void list_existing_user(user_t *a);
void add_user(user_t **users, char *name, char *pass);
struct user_details * find_user(user_t *users, char *to_find);

int main(){
    static char username[100], c_password[100], check_username[100], check_password[100], new_username[100], new_password[100];
    int main_option, log_in_option, pass_check=0;
    user_t current_user, *sample;
    printf("=====================================================================\n\n");
    printf("                        Supermarket Main Menu\n\n");
    printf("=====================================================================\n\n\n");

    log_in:
    printf("=====================================================================\n\n");
    printf("                             Login/Sign Up\n\n");
    printf("=====================================================================\n\n");
    printf("                  |         1.)  Sign Up          |\n");
    printf("                  |         2.)  Log In           |\n");
    printf("                  |         3.)  Exit             |\n");
    printf("\n=====================================================================\n\n");
    printf("                       Enter your choice: ");
    scanf("%d", &log_in_option);

    switch (log_in_option)
    {

    //ISSUE ARRAY OF user_database N̶O̶T̶ WOKRING!!!!!!!!!
    case 1:
        printf("=====================================================================\n\n");
        printf("                Enter your new username: ");
        scanf("%s", new_username);
        if (!strcmp(new_username, "ani")) break;
        if(find_user(user_database, new_username)->acc_no==-1){
        printf("                Enter your new password: ");
        scanf("%s", new_password);
        printf("\n");
        printf("=====================================================================\n\n\n");
        
        add_user(&user_database, new_username, new_password);
        current_user = *user_database;
        }

        else{
            invalid();
            goto log_in;
        }
        // //E̶R̶A̶S̶E̶ T̶H̶I̶S̶!̶!̶!̶!̶!̶!̶!̶
        // add_user(&user_database, "lol", "123");
        // add_user(&user_database, "pop", "12");
        // add_user(&user_database, "kek", "1a");
        break;
    
    case 2:
        printf("=====================================================================\n\n");
        printf("\n                      Enter your username: ");
        scanf("%s", check_username);

        list_existing_user(user_database);
        
        sample = find_user(user_database, check_username);

        printf("\nSample:");
        printf("%d %s %s", sample->acc_no, sample->username, sample->password);
        if (sample->acc_no!=-1){
            password_count_check:
            if(pass_check<=3){
                printf("                    Enter your password: ");
                scanf("%s", check_password);
                printf("\n");
                printf("=====================================================================\n\n\n");
                if (strcmp(sample->password, check_password)) {
                    invalid();
                    goto password_count_check;
                }
                pass_check++;
            }
            else {
                invalid();
                goto log_in;
            }
        }
        else {
            invalid();
            goto log_in;
        }

        current_user = *sample;
        break;

        case 3:
            quit();
            return 0;

        default:
            invalid();
            break;
    }

    main_menu: 
    printf("                    What would you like to access?\n\n");
    printf("=====================================================================\n");
    printf("                  |         1.)  Administrator    |\n");
    printf("                  |         2.)  Customer         |\n");
    printf("                  |         3.)  Log Out          |\n");
    printf("                  |         4.)  Exit             |\n");
    printf("=====================================================================\n\n");

    printf("                       Enter your choice: ");
    scanf("%d", &main_option);
    
    //Selecting an option
    switch (main_option)
    {
    case 1:
        administrator();
        goto main_menu;
        break;

    case 2:
        customer(current_user);
        goto main_menu;
        break;
    
    case 3:
        goto log_in;
        break;

    case 4:
        quit();
        return 0;

    default:
        invalid();
        goto main_menu;
        break;
    }
    return 0;
}

void add_user(user_t **users, char *name, char *pass){
    static int num = 1;
    user_t *new_node = (user_t*) malloc(sizeof(user_t));
    user_t *last = *users;
    new_node->acc_no = num; 
    strcpy(new_node->username, name); 
    strcpy(new_node->password, pass);
    //Checking if its first element
    if (*users == NULL){
        new_node->next = NULL;
        *users = new_node;
    }
    //Adding elements (LINKED LIST)
    // else {
    //     while (last!=NULL)
    //     {
    //         last = last->next;
    //     }
    //     last->next = new_node;
    // }

    //THIS ONE WORKS BTW!!!!!
    else {
        new_node->next = *users;
        (*users) = new_node;
    }
    num++;
}

void list_existing_user(user_t *a){
    while (a!=NULL)
    {
        printf("%d %s %s\n", a->acc_no, a->username, a->password);
        a = a->next;
    }
    // printf("%d %s %d\n", a->roll, a->name, a->sem);
}

struct user_details * find_user(user_t *users, char *to_find) {
    while (users!=NULL)
    {
        if (!strcmp(users->username, to_find)){
            printf("\n\n%d %s %s", users->acc_no, users->username, users->password);
            return users;
        }
        users = users->next;
    }
    return &error_details;
}